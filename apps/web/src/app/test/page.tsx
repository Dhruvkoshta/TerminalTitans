...
type Action =
	...
	| {
			type: "focusUpdate";
			payload: { penaltyPerSecond: number; status: State["focus_status"] };
	  }
	...
	switch (action.type) {
		...
		case "tick": {
			...
			return {
				...
				minutes,
				seconds,
				focus_score: Math.max(0, Math.round(focus_score)),
			};
		}
		case "setPrevLogs":
		...
		case "focusUpdate": {
			const next = Math.max(
				0,
				state.focus_score - action.payload.penaltyPerSecond
			);
			return {
				...state,
				focus_score: next,
				focus_status: action.payload.status,
			};
		}
		default:
			return state;
...
function TestPageInner() {
	...
	const initial: State = useMemo(
		() => ({
			...
			form_link: params.get("exam_link") || "",
	...
	const showToast = (key: string, title: string, description?: string) => {
		const now = Date.now();
		if (
	...
	// Fetch exam link if not provided in URL
	useEffect(() => {
		if (!state.form_link && state.exam_id) {
			fetch(
				`/api/exams/examByCode?exam_code=${encodeURIComponent(state.exam_id)}`
			)
				.then((r) => (r.ok ? r.json() : null))
				.then((exam) => {
					if (exam?.examLink) {
						dispatch({ type: "init", payload: { form_link: exam.examLink } });
					}
				})
				.catch(() => {});
		}
	}, [state.form_link, state.exam_id]);
	...
	useEffect(() => {
		const interval = setInterval(() => {
			...
			if (state.minutes === 1 && state.seconds === 0) {
				toast.warning(
					"Only 1 Minute Left, Please Submit or attendance won't be marked"
				);
			}
			...
			<div className='flex flex-col gap-3'>
				<div className='flex-1 min-h-0 rounded border flex items-center justify-center relative'>
					<Detection
						...
						onFocusUpdate={({ penaltyPerSecond, status }) =>
							dispatch({
								type: "focusUpdate",
								payload: { penaltyPerSecond, status },
							})
						}
					/>
				</div>
				...
				<div className='grid grid-cols-2 gap-2'>
					...
					<div className='rounded border p-3 text-center'>
						...
						<p className='mt-2'>
							Focus Score:{" "}
							<strong>{Math.max(0, Math.round(state.focus_score))}</strong> /
							100 &nbsp;|&nbsp; Status: <strong>{state.focus_status}</strong>
						</p>
					</div>
				</div>
				...
				<Card className='rounded-2xl'>
					<CardHeader className='flex items-center justify-between'>
						<div>
							...
							<CardDescription className='text-muted-foreground'>
								Proctoring session in progress — follow the instructions and
								keep your camera on.
							</CardDescription>
						</div>
					...
					<CardFooter>
						<p className='text-sm text-muted-foreground'>
							Your activity will be monitored during this exam.
						</p>
					</CardFooter>
				</Card>
			</div>
