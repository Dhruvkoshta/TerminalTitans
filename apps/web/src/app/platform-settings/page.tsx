...
export default function PlatformSettingsPage() {
	...
	function generateApiKey() {
		...
		const newKey = {
			...
			key: `sk_...${Math.random().toString(36).slice(2, 6)}`,
		};
		...
		<div className='p-6'>
			<div className='flex items-center justify-between mb-6'>
				<div>
					...
					<p className='text-sm text-muted-foreground mt-1'>
						Configure platform-wide options for PROCTO.
					</p>
				</div>
			...
			<div className='grid grid-cols-12 gap-6'>
				<aside className='col-span-3'>
					<nav className='sticky top-6 space-y-3 bg-card p-4 rounded-lg shadow-md text-card-foreground border border-border'>
						{tabs.map((tab) => {
							const Icon = tab.icon;
					...
					{active === "general" && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Branding and basic platform identity.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className='grid grid-cols-2 gap-6'>
									...
									<div className='space-y-4'>
										<div>
											...
											<div className='flex items-center gap-3'>
												<input
													type='file'
													accept='image/*'
													onChange={(e) => handleFileInput(e, setLogoDark)}
												/>
												{logoDark ? (
										...
										<div>
											...
											<div className='flex items-center gap-3'>
												<input
													type='file'
													accept='image/*'
													onChange={(e) => handleFileInput(e, setLogoLight)}
												/>
												{logoLight ? (
													<span className='text-sm text-foreground'>
														{logoLight.name}
										...
										<div className='mt-6 text-sm text-muted-foreground'>
											Tip: Use a transparent PNG for logos to work on both
											backgrounds.
										</div>
									</div>
								</div>
					...
					{active === "auth" && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Manage sign-in methods, password policy, and roles.
								</CardDescription>
							</CardHeader>
							<CardContent>
										...
										<div className='grid grid-cols-2 gap-4'>
											<div className='flex items-center justify-between'>
												...
												<input
													...
													onChange={(e) => {
														setEnableGoogle(e.target.checked);
														markDirty();
													}}
												/>
											</div>
											...
											<div className='flex items-center justify-between'>
												<div>
													...
													<p className='text-sm text-muted-foreground'>
														Allow users to authenticate with Microsoft accounts.
													</p>
												</div>
												<input
													type='checkbox'
													...
													onChange={(e) => {
														setEnableMicrosoft(e.target.checked);
														markDirty();
													}}
												/>
											</div>
											...
											<div className='col-span-2'>
												...
												<Input
													placeholder='e.g., myuniversity.edu'
													value={forceSSODomain}
													onChange={(e) => {
														setForceSSODomain(e.target.value);
														markDirty();
													}}
												/>
											</div>
											<div className='flex items-center gap-3'>
												<input
													...
													checked={requireCase}
													onChange={(e) => {
														setRequireCase(e.target.checked);
														markDirty();
													}}
												/>
											...
											<div className='flex items-center gap-3'>
												<input
													type='checkbox'
													checked={requireNumberSymbol}
													onChange={(e) => {
														setRequireNumberSymbol(e.target.checked);
														markDirty();
													}}
												/>
												...
												<tbody>
													<tr className='border-t border-border'>
														...
														<td className='p-3 text-muted-foreground'>
															Full access to platform settings and user
															management.
														</td>
													</tr>
													<tr className='border-t border-border'>
														...
														<td className='p-3 text-muted-foreground'>
															Create exams, manage students and view reports.
														</td>
													</tr>
													<tr className='border-t border-border'>
														<td className='p-3 font-medium'>Student</td>
														<td className='p-3 text-muted-foreground'>
															Take exams and view personal results.
														</td>
													</tr>
												</tbody>
											</table>
					...
					{active === "integrations" && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Connect PROCTO with external services and manage API keys.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<section className='space-y-4'>
									...
									<p className='text-sm text-muted-foreground'>
										Configure PROCTO with your Learning Management System
										(Moodle, Canvas, etc.).
									</p>
									...
									<div className='mt-3'>
										<Button variant='outline' onClick={resetLTIKeys}>
											Reset Keys
										</Button>
									</div>
									...
									<div className='mt-4'>
										...
										<div className='flex gap-4'>
											<div className='flex items-center gap-2'>
												<input
													type='checkbox'
													checked={webhookEvents.examCompleted}
													onChange={() => toggleWebhookEvent("examCompleted")}
												/>{" "}
											...
											<div className='flex items-center gap-2'>
												<input
													type='checkbox'
													checked={webhookEvents.studentEnrolled}
													onChange={() => toggleWebhookEvent("studentEnrolled")}
												/>{" "}
											...
											<div className='flex items-center gap-2'>
												<input
													type='checkbox'
													checked={webhookEvents.examStarted}
													onChange={() => toggleWebhookEvent("examStarted")}
												/>{" "}
					...
					{active === "billing" && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Manage subscription and view platform usage.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className='grid grid-cols-2 gap-6'>
									<div className='space-y-4'>
										<div className='rounded border p-4'>
											...
											<p className='text-sm text-muted-foreground mt-1'>
												You are on the <strong>Enterprise Plan</strong>.
											</p>
											<div className='mt-3'>
												<a
										...
										<div className='rounded border p-4'>
											...
											<div className='mt-4 space-y-4'>
												<UsageRow
													...
													used={usage.proctoredExams.used}
													limit={usage.proctoredExams.limit}
												/>
												<UsageRow
													label='Active Students'
													used={usage.activeStudents.used}
													limit={usage.activeStudents.limit}
												/>
												<UsageRow
													label='API Calls'
									...
									<div className='space-y-4'>
										<div className='rounded border p-4 text-center'>
											<SparklesIcon className='mx-auto w-10 h-10 text-primary' />
											<h4 className='font-medium mt-2'>Optimize Your Plan</h4>
											<p className='text-sm text-muted-foreground mt-1'>
												Contact sales to adjust limits or add custom
												integrations.
											</p>
											<div className='mt-3'>
...
function UsageRow({
	...
	return (
		<div>
			...
			<div className='w-full bg-secondary rounded h-3 mt-2 overflow-hidden'>
				<div className='h-3 bg-primary' style={{ width: `${percent}%` }} />
			</div>
		</div>
	);
}
