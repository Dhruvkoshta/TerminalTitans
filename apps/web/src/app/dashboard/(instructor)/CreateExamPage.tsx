...
export default function ExamCreatorPage({
	...
	async function createExam(status: "draft" | "published") {
		try {
			...
			setBusy(true);
			const payload = exportExamPayload(status);
			const res = await fetch("/api/exams/createAeiis", {
				method: "POST",
			...
			toast.success(
					...
					: "Draft saved successfully!",
				{
					description:
						data.message ||
						(data.shareUrl ? `Share link: ${data.shareUrl}` : undefined),
				}
			);
		} catch (e: any) {
			console.error("Error creating exam:", e);
			toast.error(e?.message || "Could not save exam");
		} finally {
			setBusy(false);
		}
	}
	...
	function saveDraft() {
		void createExam("draft");
	}

	function publishExam() {
		void createExam("published");
	}
				...
				<div className='mb-6 flex items-center justify-between'>
						...
						<div className='mr-4 flex-1'>
							<div className='flex items-center gap-2 rounded-md border bg-white px-3 py-2 text-sm shadow-sm'>
								...
								<button
									onClick={() => {
										navigator.clipboard
											.writeText(`${location.origin}${shareUrl}`)
											.then(() => toast.success("Copied share link"));
									}}
									className='ml-auto inline-flex items-center gap-1 rounded border px-2 py-1 text-xs hover:bg-slate-50'
								>
									Copy
								</button>
							</div>
					...
					<div className='flex items-center gap-3'>
						<button
							onClick={saveDraft}
							disabled={busy}
							className='inline-flex items-center gap-2 px-4 py-2 rounded-md bg-white border text-slate-700 hover:bg-slate-50 shadow-sm disabled:opacity-60'
						>
						...
						<button
							onClick={publishExam}
							disabled={busy}
							className='inline-flex items-center gap-2 px-4 py-2 rounded-md bg-slate-800 text-white hover:bg-slate-900 shadow disabled:opacity-60'
						>
