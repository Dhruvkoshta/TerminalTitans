...
export default function Detection(props: DetectionProps) {
	...
	const showToast = useCallback(
		(key: string, title: string, description: string) => {
			const now = Date.now();
			if (
				!lastToastTime.current[key] ||
				now - lastToastTime.current[key] >= TOAST_COOLDOWN
			) {
	...
	const runGaze = useCallback(async () => {
		...
		try {
			...
			if (leftIris && rightIris && leftInner && rightOuter) {
				...
				const inter = Math.hypot(
					rightOuter.x - leftInner.x,
					rightOuter.y - leftInner.y
				);
				if (prevEyeCenter.current && prevInterOcDist.current) {
					const dx =
						(cx - prevEyeCenter.current.x) / (prevInterOcDist.current || 1);
					const dy =
						(cy - prevEyeCenter.current.y) / (prevInterOcDist.current || 1);
	...
	useEffect(() => {
		...
		async function loadAll() {
			...
			try {
				...
				try {
					console.warn(
						"MediaPipe detector failed, trying tfjs runtime:",
						mediapipeErr
					);
					faceDetectorRef.current = await fld.createDetector(
						fld.SupportedModels.MediaPipeFaceMesh,
					...
					console.error(
						"Both MediaPipe and tfjs detector creation failed:",
						mediapipeErr,
						tfjsErr
					);
		...
		loadAll().catch((err: any) => {
			...
			toast.error("Failed to load ML models", {
				description:
					(err && err.message) ||
					"Please refresh, allow camera access, and check network/CSP.",
			});
		});
